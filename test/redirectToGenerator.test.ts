...
import { replaceDynamicValues } from '../src/utils/replaceDynamicValues.js';
import { InterfaceRouterStore } from '../src/types/InterfaceRouterStore.js';
import { TypeRoute } from '../src/types/TypeRoute.js';
import { TypeRouteWithParams } from '../src/types/TypeRouteWithParams.js';
...
function checkCurrent(routerStore: InterfaceRouterStore<any>, route: TypeRouteWithParams) {
  ...
  if ('props' in route) {
    // @ts-ignore
    expectedObj.props = route.props;
  }

  expect(routerStore.currentRoute).to.deep.eq(expectedObj);
